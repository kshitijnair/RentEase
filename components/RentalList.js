...
import { collection, query,addDoc, onSnapshot, where, doc, setDoc, arrayUnion, arrayRemove,getDocs } from 'firebase/firestore';
...
const RentalList = () => {
  ...
  const toggleBookmark = async (listingId) => {
    ...
    if (querySnapshot.empty) {
      ...
      if (listingIds.includes(listingId)) {
        await setDoc(bookmarkDocRef, { listingIds: arrayRemove(listingId) }, { merge: true });
        setBookmarks(listingIds.filter((id) => id !== listingId));
      } else {
        await setDoc(bookmarkDocRef, { listingIds: arrayUnion(listingId) }, { merge: true });
  ...
  const renderListings = () => {
    if (activeTab === 'home') {
      return <RentalListItem data={listings} bookmarks={bookmarks} toggleBookmark={toggleBookmark} />;
    } else {
      const bookmarkedData = listings.filter((listing) => bookmarks.includes(listing.id));
      return <RentalListItem data={bookmarkedData} bookmarks={bookmarks} toggleBookmark={toggleBookmark} />;
    }
  };
...
>
  <Text style={styles.tabText}>All Listings</Text>
        ...
        <TouchableOpacity
          style={[styles.tab, activeTab === 'bookmarks' ?  styles.activeTab : styles.inactiveTab,]}
